export function parseUsageInfo(data, colElements, cleanerFn) {
    ...
    switch (colIndex) {
      ...
      case 0:
        if (formattedElement.split(' ').length === 1) {
          return (
            data[data.length - 1][colIndex].split(' ')[0] +
            ' ' +
            formattedElement
          )
        }
...
export function parseHistoryInfo(data, colElements, cleanerFn) {
