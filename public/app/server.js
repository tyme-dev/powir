...
function exportJSONData() {
  ...
  dialog
    ...
    .then((result) => {
      batteryReport
        .then((data) =>
          writeDataToFile(JSON.stringify(data, null, 4), result.filePath)
        )
        .catch((error) => log('error', error))
...
function exportPDFReport() {
  ...
  let currentWindow = BrowserWindow.getAllWindows().filter((window) =>
    window.isVisible()
  )[0]
  let options = {
  ...
  currentWindow.webContents
    ...
    .then((data) => {
      dialog
        .showSaveDialog(null, options)
        .then((result) => {
          writeDataToFile(data, result.filePath)
            .then((_) => log('PDF saved!'))
            .catch((error) => log('error', error))
        })
        .catch((err) => log(err))
    })
